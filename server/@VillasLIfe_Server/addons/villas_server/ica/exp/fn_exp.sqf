...
if (_param == "menu") exitWith {
	...
	_ramas = ["Medicina", "Conducción", "Fortaleza", "Minería", "Pesca", "Agricultura", "Logística", "Artesanía", "Informática", "Rebelde", "Las Sombras"];
...
if (_param == "rellena") exitWith {
	...
	_habilidades = switch (_rama) do {
		...
		case "min": {["Sin esperas", "Recoger Doble"]};
		...
		case "tir": {["Armas Medias", "Armas Altas", "Materiales"]};
...
if (_param == "informa") exitWith {
	...
	_txt = switch (_habilidad) do {
		...
		case "for7": {"Ahora puedes quitarte las esposas."};
		
		case "min1": {"Picas más rapido, sin esperas"};
		case "min2": {"Ahora picas más el doble de una picada"};
		//case "min3": {"Ahora cobras un extra por cada viaje que hagas"};
		...
		case "art4": {"Puedes fabricar un bisturí y un traje de buzo"};
		case "art5": {"Puedes fabricar bandas de clavos."};
		...
		case "art8": {"Puedes fabricar polvora"};
		...
		case "inf2": {"Puedes acceder a las cámaras de seguridad de la policía y ver las matriculas de los demás. Pd : No hay cámaras en los baños."};
		...
		case "tir1": {"Puedes comprar Chalecos y alguna que otra pistola, poca protección te darán estas."};
		case "tir2": {"Puedes comprar Chalecos y pistolas algo más avanzados, podrás defenderte perfectamente."};
		case "tir3": {"Podrás comprar materiales y algún que otro explosivo, nada ilegal ejem ejem..."};
...
if (_param isEqualTo "aprende") exitWith {
	...
	//if (vicio_din < _din) exitWith {hint format ["Por cada SP que gastes tienes que pagar 2.000€ \nAhora necesitas %1€", _din]};
