...
if (_param isEqualTo "Init") exitWith {
    _npc addAction
    [
        "<t color=""#DE2828"">" + "Robar Blas de Lezo",
        {
            ["Robo"] spawn ica_fnc_robolezo;
        },
        [],
        1.5,
        true,
...
if (_param isEqualTo "Robo") exitWith {
    ...
    [10,"Alarma - Robo",[1,0,0,1],2,("Están robando el Blas de Lezo"),[1,1,1,1],1.5] remoteexec ["ica_fnc_anuncio",west];

    _Pos = position player;
    _MarkerN = format ["%1_Marker",[_npc] call BIS_fnc_objectVar];
    _MarkerT = "!ATENCIÓN! Están Robando el Blas de Lezo";
    ...
    hintSilent parseText format ["<t size = ""2"" color=""#DE2828"" align='center' >Robo a Blas de Lezo<br/>
    ...
    ["lezo",_npc] remoteexec ["ica_fnc_check", 2];

    if (_fuera) exitWith {};

    _polisOn = {side _x == WEST} count playableUnits;

    _din = (800000 + (1000 * _polisOn));
    vicio_din = _din + vicio_din;
    hint format ["En el Robo a el Blas de Lezo se ha obtenido %1€ ", _din];
};
