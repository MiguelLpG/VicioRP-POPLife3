...
0 spawn {
	while {true} do {
		sleep (60 * 1); // 5/min 10/2min 300/h 900/3h
		
		_e = (experiencia select 2);
		if (_e < 1500) then {
			["ene", 75] call ica_fnc_arrayExp;
			if (_e > 1500) then {experiencia set [2, 1000]};
		};
	};
};


while { sleep 240; playerSide == west } do {
 ["exp", 5] call ica_fnc_arrayExp;
};

while { sleep 240; playerSide == civilian } do {
 ["exp", 2] call ica_fnc_arrayExp;
};

while { sleep 240; playerSide == independent } do {
 ["exp", 5] call ica_fnc_arrayExp;
};
...
/*if ((side player == INDEPENDENT)) then {
	...
	0 spawn {
		while {true} do {
			if (({side _x == INDEPENDENT} count (nearestObjects [player, ["Man"], 10])) >= 1) then {["exp", 5] call ica_fnc_arrayExp};
