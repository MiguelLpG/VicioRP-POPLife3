...
if (Manolo getVariable ["Robo", false]) exitWith {hintSilent "El robo esta en progreso!"};
...
if(_policias < 2) exitWith{hintSilent "Hacen falta 2 policias";};
if ((robosactivos + 1.5) >= 3) exitWith {hint "Hay muchos robos en progreso, espera a que terminen"};
if (Manolo getVariable ["Robo", false]) exitwith {hint "Ya esta robando tu colega, ayudale"};

_metros_cancelar_robo = 15;
_rip = true;

_dinero = 50000 + round(random 10000);


_chance = random(100);


if(_chance >= 0) then {[1,format["Alarma! - manolo: %1 Esta siendo robado!", _shop]] remoteExec ["life_fnc_broadcast",west]};
[10,"Robo",[1,0,0,1],2,("Alarma! - manolo: Esta siendo robado!"),[1,1,1,1],1.5] remoteexec ["ica_fnc_anuncio",west];
//"AlarmCar" remoteExec ["playSound",west];
Manolo setVariable ["Robo", true, true];
[32,Manolo,1] remoteExec ["ica_fnc_robodesconexion",2];
...
if(_rip) then{
	while{true} do {
		...
		_Pos = position player;
										_marker = createMarker ["Manoloo", _Pos];
										"Manoloo" setMarkerColor "ColorRed";
										"Manoloo" setMarkerText "!!ATENCION!!Estan Robando aqui!";
										"Manoloo" setMarkerType "mil_warning";
		if(_cP >= 1) exitWith {};
		if !(alive _ladron) exitWith {deleteMarker "Manoloo";_rip = false;5 cutText ["","PLAIN"];};
		if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {};

	};

	if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {deleteMarker "Manoloo"; hintSilent "Necesitas estar a menos de 15 metros de el dependiente! - El dependiente a bloqueado la caja registradora"; 5 cutText ["","PLAIN"]; _rip = false; };
	_dineroextra = _dinero + (_dinero * 0.25);
	if!(alive _ladron) exitWith {_cp = 1;deleteMarker "Manoloo";_rip = false;5 cutText ["","PLAIN"];};
	5 cutText ["","PLAIN"];

	titleText[format["Has robado %1 € hasta la proxima",[_dinero] call life_fnc_numberText],"PLAIN"];

	deleteMarker "Manoloo";
