...
if (RoboCasinoaa getVariable ["Robo", false]) exitWith {hintSilent "El robo esta en progreso!"};
...
if(_policias < 12) exitWith{hintSilent "No hay 12 policias, no es divertido!";};
if ((robosactivos + 1.5) >= 3) exitWith {hint "Hay muchos robos en progreso, espera a que terminen"};

if (RoboCasinoaa getVariable ["Robo", false]) exitwith {hint "Ya esta robando tu colega, ayudale"};
...
if(_chance >= 0) then {[1,format["Alarma! - Casino: %1 Esta siendo robado!", _shop]] remoteExec ["life_fnc_broadcast",west]};
[10,"Robo",[1,0,0,1],2,("Alarma! - Casino: Esta siendo robado!"),[1,1,1,1],1.5] remoteexec ["ica_fnc_anuncio",west];
//"AlarmCar" remoteExec ["playSound",west];
RoboCasinoaa setVariable ["Robo", true, true];
[32,RoboCasinoaa,1] remoteExec ["ica_fnc_robodesconexion",2];
...
_Pos = position player;
				                _marker = createMarker ["Robo_Casino", _Pos];
				                "Robo_Casino" setMarkerColor "ColorRed";
				                "Robo_Casino" setMarkerText "!!ATENCION!!Estan Robando aqui!";
				                "Robo_Casino" setMarkerType "mil_warning";
if(_cP >= 1) exitWith {};
if !(alive _ladron) exitWith {deleteMarker "Robo_Casino";_rip = false;5 cutText ["","PLAIN"];};
if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {};
};

if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {deleteMarker "Robo_Casino"; hintSilent "Necesitas estar a menos de 10 metros de el dependiente! - El dependiente a bloqueado la caja registradora"; 5 cutText ["","PLAIN"]; _rip = false; };
_dineroextra = _dinero + (_dinero * 0.25);
if!(alive _ladron) exitWith {_cp = 1;deleteMarker "Robo_Casino";_rip = false;5 cutText ["","PLAIN"];};
5 cutText ["","PLAIN"];

titleText[format["Has robado %1 € del casino",[_dinero] call life_fnc_numberText],"PLAIN"];


deleteMarker "Robo_Casino";
