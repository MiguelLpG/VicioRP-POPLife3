...
if (Lezo getVariable ["Robo", false]) exitWith {hintSilent "El robo esta en progreso!"};
...
if(_policias < 13) exitWith{hintSilent "No hay 13 policias, no es divertido!";};
if ((robosactivos + 1.5) >= 3) exitWith {hint "Hay muchos robos en progreso, espera a que terminen"};

if (Lezo getVariable ["Robo", false]) exitwith {hint "Ya esta robando tu colega, ayudale"};
...
if(_chance >= 0) then {[1,format["Alarma! - Blas de Lezo: %1 Esta siendo robado!", _shop]] remoteExec ["life_fnc_broadcast",west]};
[10,"Robo",[1,0,0,1],2,("Alarma! - Blas de Lezo: Esta siendo robado!"),[1,1,1,1],1.5] remoteexec ["ica_fnc_anuncio",west];
//"AlarmCar" remoteExec ["playSound",west];
Lezo setVariable ["Robo", true, true];
[32,Lezo,1] remoteExec ["ica_fnc_robodesconexion",2];
...
_Pos = position player;
				                _marker = createMarker ["Lezo", _Pos];
				                "Lezo" setMarkerColor "ColorRed";
				                "Lezo" setMarkerText "!!ATENCION!!Estan Robando aqui!";
				                "Lezo" setMarkerType "mil_warning";
if(_cP >= 1) exitWith {};
if !(alive _ladron) exitWith {deleteMarker "Lezo";_rip = false;5 cutText ["","PLAIN"];};
if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {};
};

if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {deleteMarker "Lezo"; hintSilent "Necesitas estar a menos de 10 metros de el dependiente! - El dependiente a bloqueado la caja registradora"; 5 cutText ["","PLAIN"]; _rip = false; };
_dineroextra = _dinero + (_dinero * 0.25);
if!(alive _ladron) exitWith {_cp = 1;deleteMarker "Lezo";_rip = false;5 cutText ["","PLAIN"];};
5 cutText ["","PLAIN"];

titleText[format["Has robado %1 € en el Blas de Lezo",[_dinero] call life_fnc_numberText],"PLAIN"];
