...
if (Paco getVariable ["Robo", false]) exitWith {hintSilent "El robo esta en progreso!"};
...
if (Paco getVariable ["Robo", false]) exitwith {hint "Ya esta robando tu colega, ayudale"};
...
if(_chance >= 0) then {[1,format["Alarma! - Paco: %1 Esta siendo robado!", _shop]] remoteExec ["life_fnc_broadcast",west]};
[10,"Robo",[1,0,0,1],2,("Alarma! - Paco: Esta siendo robado!"),[1,1,1,1],1.5] remoteexec ["ica_fnc_anuncio",west];
//"AlarmCar" remoteExec ["playSound",west];
Paco setVariable ["Robo", true, true];
[32,Paco,1] remoteExec ["ica_fnc_robodesconexion",2];
...
if(_rip) then{
	while{true} do {
		...
		_Pos = position player;
										_marker = createMarker ["Pacoo", _Pos];
										"Pacoo" setMarkerColor "ColorRed";
										"Pacoo" setMarkerText "!!ATENCION!!Estan Robando aqui!";
										"Pacoo" setMarkerType "mil_warning";
		if(_cP >= 1) exitWith {};
		if !(alive _ladron) exitWith {deleteMarker "Pacoo";_rip = false;5 cutText ["","PLAIN"];};
		if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {};

	};

	if((_ladron distance _shop > _metros_cancelar_robo) || (!isPlayer _ladron)) exitWith {deleteMarker "Pacoo"; hintSilent "Necesitas estar a menos de 15 metros de el dependiente! - El dependiente a bloqueado la caja registradora"; 5 cutText ["","PLAIN"]; _rip = false; };
	_dineroextra = _dinero + (_dinero * 0.25);
	if!(alive _ladron) exitWith {_cp = 1;deleteMarker "Pacoo";_rip = false;5 cutText ["","PLAIN"];};
	5 cutText ["","PLAIN"];
