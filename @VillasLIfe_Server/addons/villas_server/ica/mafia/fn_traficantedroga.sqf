...
if (_param isEqualTo "menu") exitWith {
	...
	if (_modo) then {
		...
		if (_tio == 1) then {
			...
			_pre1 ctrlSetText format ["%1 €",round (1500/_mult)];
			_pre2 ctrlSetText format ["%1 €",round (1800/_mult)];
			_pre3 ctrlSetText format ["%1 €",round (1000/_mult)];
			_pre4 ctrlSetText format ["%1 €",round (1000/_mult)];
			_bar1 progressSetPosition (traf1Mar / _maxTraf);
			_bar2 progressSetPosition (traf1Her / _maxTraf);
			_bar3 progressSetPosition (traf1Coc / _maxTraf);
			_bar4 progressSetPosition (traf1Met / _maxTraf);
...
if (_param isEqualTo "compro") exitWith {
	...
	if !(vicio_din >= _pre) exitWith {hint "No tienes dinero para pagarme."};
...
// ["mochila_grande", 1] call ica_fnc_item; ["pop_cannabis_b", 5] call ica_fnc_item; vicio_din = 1000000;