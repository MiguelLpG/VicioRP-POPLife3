...
if (_param isEqualTo "Init") exitWith {
    ...
    _npc addAction
    [
        "<t color=""#DE2828"">" + "Robar el Banco",
        {
            ["Robo"] spawn ica_fnc_robobanco;
        },
        [],
        1.5,
        true,
...
if (_param isEqualTo "Robo") exitWith {
    ...
    [10,"Alarma - Robo",[1,0,0,1],2,("Están robando el Banco"),[1,1,1,1],1.5] remoteexec ["ica_fnc_anuncio",west];

    _Pos = position player;
    _MarkerN = format ["%1_Marker",[_npc] call BIS_fnc_objectVar];
    _MarkerT = "!ATENCIÓN! Están Robando el Banco";
    ...
    hintSilent parseText format ["<t size = ""2"" color=""#DE2828"" align='center' >Robo a Banco<br/>
    ...
    ["banco",_npc] remoteexec ["ica_fnc_check", 2];

    if (_fuera) exitWith {};

    _polisOn = {side _x == WEST} count playableUnits;

    _din = (1000000 + (1000 * _polisOn));
    vicio_din = _din + vicio_din;
    hint format ["En el Robo a el Banco se ha obtenido %1€ ",_din];
};
