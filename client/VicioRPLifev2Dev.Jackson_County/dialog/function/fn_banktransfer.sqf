...
if(_val > vicio_atmdin) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > vicio_atmdin) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

vicio_atmdin = vicio_atmdin - (_val + _tax);
