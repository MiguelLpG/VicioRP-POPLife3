	...
	switch (_mode) do
	{
		...
		{	
			if (!life_perksInitialized) exitWith {
				["Your perks are still being loaded by the server, please try again later",true,"slow"] call ANZUS_fnc_notificationSystem;
			};	
		...
		case "onButtonPurchaseClick":
		{
			...
			["Perk purchased",true,"slow"] call ANZUS_fnc_notificationSystem;
