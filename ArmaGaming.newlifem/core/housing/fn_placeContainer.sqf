...
if (!isNull _house) then {
    _message = 1;
    if (([player] call ANZUS_fnc_playerInBuilding) && {([_container] call ANZUS_fnc_playerInBuilding)}) then {
        _message = 2;
...
[true,_type,1,"container"] call ANZUS_fnc_handleInv;

if (_message isEqualTo 0 || _message isEqualTo 1) then {
    [localize "STR_House_Container_House_Near",true,"slow"] call ANZUS_fnc_notificationSystem;
};
if (_message isEqualTo 2) then {
    [localize "STR_House_Container_House_Near_Owner",true,"slow"] call ANZUS_fnc_notificationSystem;
};
if (_message isEqualTo 3) then {
    [localize "STR_House_Container_Floating",true,"slow"] call ANZUS_fnc_notificationSystem;
};
if (_message isEqualTo 4) then {
    [localize "STR_ISTR_Box_HouseFull",true,"slow"] call ANZUS_fnc_notificationSystem;
};