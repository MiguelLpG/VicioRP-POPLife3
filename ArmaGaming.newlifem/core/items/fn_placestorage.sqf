...
detach _container;
[_container,true] remoteExecCall ["ANZUS_fnc_simDisable",RANY];
...
if ((typeOf _container) == "B_supplyCrate_F") then {
    [false,"storagebig",1,"container"] call ANZUS_fnc_handleInv;
} else {
    [false,"storagesmall",1,"container"] call ANZUS_fnc_handleInv;
};

[_container, _isFloating] call ANZUS_fnc_placeContainer;
