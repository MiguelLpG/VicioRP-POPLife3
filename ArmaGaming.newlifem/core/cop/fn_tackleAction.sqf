...
if(_totald >= -90 && _totald < 1 || _totald <= 90 && _totald > -1 || _totald >= 270 || _totald <= -270) then {
	...
	detach player;	
	[_target,profileName] remoteExecCall ["ANZUS_fnc_crashTackled",_target];
	uiSleep 3;
	life_knockout = false;
	deleteVehicle _obj;

} else {
	["You must be behind your target to tackle them.",false,"slow"] call ANZUS_fnc_notificationSystem;
};
