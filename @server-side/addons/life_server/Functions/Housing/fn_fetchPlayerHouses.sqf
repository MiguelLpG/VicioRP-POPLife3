...
_containers = [_query,2,true] call ANZUS_fnc_asyncCall;
...
_containerss = [];
{
    ...
    _trunk = [_x select 3] call ANZUS_fnc_mresToArray;
    if (_trunk isEqualType "") then {_trunk = call compile format ["%1", _trunk];};
    _gear = [_x select 4] call ANZUS_fnc_mresToArray;
...
_houses = [_query,2,true] call ANZUS_fnc_asyncCall;
