...
_query = format ["SELECT id, property_tax FROM houses WHERE pos='%1' AND pid='%2' AND owned='1'",_housePos,_uid];
_queryResult = [_query,2] call ANZUS_fnc_asyncCall;
	
_house setVariable ["house_id",(_queryResult select 0),true];
_house setVariable ["house_tax",(_queryResult select 1),true];