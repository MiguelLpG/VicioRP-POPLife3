...
if((["pop_tarjeta"] call ica_fnc_tengo == 0) && (vicio_din < _price)) exitWith {hint "No tiene suficiente dinero en efectivo y no tiene tarjeta de credito !"};
...
{
if((["pop_tarjeta"] call ica_fnc_tengo == 0) && (vicio_din < _price)) exitWith {hint "No tiene suficiente dinero en efectivo y no tiene una tarjeta de credito !"};
if((_gangbank >= _prixcb) && (["pop_tarjeta"] call ica_fnc_tengo > 0)) then {
        _action = [
            format["Dinero de la Mafia : <t color='#8cff9b'>%1$</t><br/>CUENTA BANCARIA : <t color='#8cff9b'>%5$</t><br/>Efectivo : <t color='#8cff9b'>%2$</t><br/><br/>Precio en Efectivo : <t color='#8cff9b'>%3$</t><br/>Precio por Trajeta : <t color='#8cff9b'>%4$</t>",
                ...
                [vicio_din] call life_fnc_numberText,
                [_price] call life_fnc_numberText,
                [_prixcb] call life_fnc_numberText,
                [vicio_atmdin] call life_fnc_numberText
            ],
    ...
    "Mi dinero"
        ...
        if(_action) then {
                ...
                if(_prixcb > vicio_atmdin) exitWith {hint "No tiene suficiente dinero en su cuenta !"};
                vicio_atmdin = vicio_atmdin - _prixcb;
                ...
                if(_price > vicio_din) exitWith {hint "No tiene suficiente dinero en efectivo !"};
                vicio_din = vicio_din - _price;
        ...
        if((["pop_tarjeta"] call ica_fnc_tengo == 0)) then {
                if(_price > vicio_din) exitWith {hint "no tiene suficiente dinero en efectivo y no dispone de una tarjeta de credito !"};
                vicio_din = vicio_din - _price;
                hint parseText format["Compro un %1 por <t color='#8cff9b'>%2$</t>",_itemInfo select 1,[_price] call life_fnc_numberText];
                [_item,true] spawn life_fnc_handleItem;
        } else {
