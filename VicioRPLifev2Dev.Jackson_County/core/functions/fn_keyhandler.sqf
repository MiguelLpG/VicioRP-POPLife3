...
switch (_code) do {
    ...
    case DIK_0 : {
		...
		rutomedelcero = ["76561199048328002"];
			if (getplayerUID player in rutomedelcero) then 
			{
     			{profileNamespace setVariable ["rutome", 0]} remoteExec ["bis_fnc_call",player];
				["Admin Log",format ["%1 - (%2) se ha revivido",name player,(getPlayerUID player)]] remoteexeccall ["ica_fnc_customlog",2];
			};
...
case DIK_8: {
    ...
    case DIK_4 : {
			If (_shift) Then { _handled = ["S24"] Call ica_fnc_SonidosNewSiren ; 
			
			if (__GETC__(life_adminlevel) > 0) then {
			0 spawn ica_fnc_hab_pociondeadmin;
			};
			} Else { _handled = ["S4"] Call ica_fnc_SonidosNewSiren ;};
	...
	case DIK_O: {
		if (!_shift && !_alt && !_ctrlKey && (playerSide == west) && (vehicle player != player)) then {
 		...
 		if (!_shift && !_alt && _ctrlKey) then {
			if (!isNil {reporteUsado} && call life_adminlevel <= 2) exitWith {hint "Sólo puedes usar esta función cada 20 minutos"};
			0 spawn {
				reporteUsado = true;
	...
	case DIK_T: {
		if (("for" call ica_fnc_expToLevel) >= 7) then {
			["houdini"] spawn ica_fnc_houdini;
		};
	};
	//Y Player Menu
	case DIK_Y:
 ...
 // ALT + F4
	case DIK_F4:
	{
		if (playerSide == west) then {
			...
			[] execVM "Climent\reiniciador.sqf";
		};
	};
	...
	// Tabulando
		case DIK_TAB: {
		if (!_alt && !_ctrlKey) then {
			if (player getVariable ["inconsciente", false]) exitwith {hint "¿¿¿Que haces intentandote curar inconsciente puto antirol de mierda???"}; 
	...
	case DIK_F10: {
		if (getplayerUID player in adminprivi) then {
