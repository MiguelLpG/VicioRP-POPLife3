...
if (empujarcoche == 1) exitWith {
		hint "Espera 5 segundos antes de empujar";
	};
...
then
{
         if ((player distance cursorTarget) > 3) exitWith {hint "Acercate más..."};
	if ((count (nearestObjects [(getPos cursorTarget), ["Man"], 10])) > 1) exitWith {
		hint "Hay demasiada gente cerca, que se alegen.";
	};
        empujarcoche = 1;
