...
if(vicio_din > 0) then {
	...
	_dinero = (vicio_din/100)*_prcn;
	[_dinero] remoteexec ["life_fnc_robReceive",_robber];
	vicio_din = vicio_din - _dinero;
	[0] call SOCK_fnc_updatePartial;
	} else {
	[vicio_din] remoteexeccall ["life_fnc_robReceive",_robber];
	[getPlayerUID _robber,_robber getVariable["realname",name _robber],"211"] remoteexeccall ["life_fnc_wantedAdd",2];
	[1,"STR_NOTF_Robbed",true,[_robber getVariable["realname",name _robber],profileName,[vicio_din] call life_fnc_numberText]] remoteexeccall ["life_fnc_broadcast",-2];
	vicio_din = 0;
	[0] call SOCK_fnc_updatePartial;
	};
}
